...
**Основные протоколы**: :ref:`TCP <section-tcp>` (надежный, с подтверждениями и контролем потока), UDP (быстрый, без гарантий доставки).
...
* Транспортный (4): :ref:`TCP <section-tcp>` разбивает данные на сегменты, пронумеровывает, добавляет порт назначения (например, 443). Начинается TCP-handshake.
...
На приеме все операции выполняются в обратном порядке (декапсуляция): физический уровень → декодирование кадра → проверка IP → сборка :ref:`TCP <section-tcp>` → расшифровка TLS → показ страницы в браузере.

****************************
Как это помогает на практике
****************************

* Диагностика сети: если ping проходит — сетевой (IP) уровень работает; если ping не проходит, но локальная сеть есть — проблема может быть на сетевом уровне.
* Понимание безопасности: фаерволы и правила часто опираются на порты (транспортный уровень) и IP-адреса (сетевой уровень).
* Проектирование сети: выбор между коммутатором и маршрутизатором, настройка VLAN — все связано с канальным и сетевым уровнями.
* Оптимизация: если сайт медленно отвечает, нужно понять, где задержка: приложение, :ref:`TCP <section-tcp>` (потеря пакетов, повторная передача), маршрутизация или физика (плохой канал).