...
.. important::
    ...
    **Закрытый ключ (private key)** — хранится только у владельца ключа на компьютере, его **никому нельзя передавать**.

    **Открытый ключ (public key)** — загружается на сервер или в сервис (например, в GitHub).

При подключении сервер сверяет ключи и подтверждает вход, если они совпадают.
